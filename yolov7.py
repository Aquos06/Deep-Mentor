...
from yolov7Util.general import non_max_suppression, set_logging, scale_coords
from yolov7Util.torch_utils import select_device, TracedModel
from yolov7Util.plots import output_to_keypoint, plot_one_box
...
#this
    def predict(self, img1):
        ...
        if self.pred:
            ...
            self.pred[:,:4] = scale_coords(self.img.shape[2:], self.pred[:,:4], img1.shape).round()
            self.pred = self.pred.cpu().tolist()
            self.person_coor = []
